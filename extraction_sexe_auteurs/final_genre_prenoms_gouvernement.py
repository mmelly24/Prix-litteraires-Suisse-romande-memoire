...
DATA_FILE_PATH = Path("./prenoms_genre.txt")
RESULTS_FILE_PATH = Path("./final_prenoms_genre_gouvernement.xslx")
...
def main():
    ...
    regex = re.compile(r"([a-z-áàéèïúó]+)(;)([a-z],?[a-z]?);(.*)")
    ...
    with RESULTS_FILE_PATH.open(mode="w", encoding="utf-8") as fichier:
        for match in matchs:
            fichier.write(f"{match.group(1)}\t{match.group(3)}\n")
