...
DATA_FILE_PATH = Path("./laureat_prix_goncourt.txt")
RESULTS_FILE_PATH = Path("./final_laureat_prix_goncourt.xslx")
...
def main():
    ...
    with RESULTS_FILE_PATH.open(mode="w", encoding="utf-8") as fichier:
        for match in matchs:
            fichier.write(f"{match.group(1)}\t{match.group(3)}\t{match.group(4)}\t{match.group(5)}\n")
